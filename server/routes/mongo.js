...
mgRouter.get("/repositories/:collection/getLastRegisterCount", async (req, res) => {
    const collection = req.params.collection;
...
mgRouter.get("/repositories/:collection/getDocsByYear/:year", async (req, res) => {
    try {
        const collection = req.params.collection;
...
mgRouter.put("/repositories/:collection/updateTimes/:id", async (req, res) => {
    ...
    // $set: {'Veracruz.$': data}
    let result = await db.collection(collection).updateOne({ id: id }, {
        $set: {
            [str]: {
